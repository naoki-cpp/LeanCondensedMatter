...
**Still needed:** `helmholtzFreeEnergy_ge`/`vonNeumannEntropy_gibbsState` (the remaining Gibbs–Klein / free-energy-minimization content built on top of `gibbsState` in the finite-dimensional file) are not yet ported. **Open ambiguity, not attempted (2026-07-14):** the finite-dimensional `helmholtzFreeEnergy_ge` states `-(1/β)·ln Z(β) ≤ energyExpValue ρ Hop - (1/β)·vonNeumannEntropy ρ` with everything `ℝ`-valued, but the infinite-dimensional `vonNeumannEntropy` (above) is `ENNReal`-valued and can genuinely be `⊤`. Porting the inequality needs a formalization choice for how the RHS's subtraction is to be read when entropy is infinite (e.g. restrict to a hypothesis that entropy is finite and use `ENNReal.toReal`, or restate the inequality in `EReal`/`ENNReal` throughout) — left to the user to decide before attempting this.
