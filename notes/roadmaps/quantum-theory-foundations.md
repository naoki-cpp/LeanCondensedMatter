...
**In progress: infinite-dimensional density operators.** Now that Track C's `ContinuousLinearMap.trace` (with linearity/cyclicity) is done, extending `DensityOperator` beyond finite dimensions is underway in `LeanCondensedMatter/QuantumTheory/DensityOperatorTraceClass.lean` (namespace `QuantumTheory.TraceClass`, additive to the finite-dimensional file above — nothing there is touched). `QuantumTheory.TraceClass.DensityOperator` (positive + compact + trace-class + trace `1`) and `isCompactOperator_rankOne` (any rank-one operator `|x⟩⟨y|` is compact, via factoring through the locally-compact `ℂ`) are proved. **A genuine mathematical gap, not just a missing Lean lemma:** porting `POVM`/`prob`/`sum_prob_eq_one` needs a trace for operators that need not be self-adjoint (`E_m ∘ ρ` for positive `E_m`, `ρ` isn't self-adjoint unless they commute) — `ContinuousLinearMap.trace` only covers the self-adjoint case. This needs either a Hilbert–Schmidt-class trace or an explicit commuting-POVM restriction; not yet attempted. **Still needed even for single-operator properties:** `pure ψ` is trace-class (needs a "finite-rank ⇒ trace-class" lemma — rank-one's eigenspace computation isn't done yet) and `purity` (`Tr[ρ²]`, well-posed since `ρ ∘ ρ` is self-adjoint whenever `ρ` is, but needs `ρ ∘ ρ`'s own compactness/trace-class facts).
