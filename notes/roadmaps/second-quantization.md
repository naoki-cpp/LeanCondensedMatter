...
| `Common/BlochDeDominicisPairing.lean` | `Common.BlochDeDominicis.Pairing n` — perfect pairings of `Fin (2 * n)` behind a stable `partner` interface, implemented by fixed-point-free involutive permutations; finite enumeration, partner laws, normalized-pair membership, crossing count, statistics weight `ζ^crossings`, and `Pairing.eraseZeroPair` for deleting position `0` and its partner via the ordered-position isomorphism; the four-position theorem gives the adjacent/crossing/nested weights `1`, `ζ`, `1` without choosing an enumeration order | `proved` |
