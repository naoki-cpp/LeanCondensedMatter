...
| B2 | `Bosonic/ImaginaryTimeEvolution.lean` — `freeEigenvalue ε n := Σᵢ n(i)·ε(i)`, `e^{τH₀}` diagonal in the occupation basis, Heisenberg-picture `imaginaryTimeEvolve`, and the evolved operators `a_i(τ) = e^{-τεᵢ} a_i`, `a_i†(τ) = e^{τεᵢ} a_i†` | `proved` |
