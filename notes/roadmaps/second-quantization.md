...
6. **Diagram connectedness and component decomposition — complete through scalar-prefactor factorization.** Built *before* any concrete
   ...
   - PR 4a/4b, `Fermionic/Diagrammatics/WickDiagram.lean` / `WickDiagramConnected.lean` — leg indexing
   ...
   - PR 5a/5b/5c — `Analysis/OrderedSimplexIntegral.lean` (recursive iterated integral, via
     ...
     `Fermionic/Diagrammatics/WickDiagram/Ordered.lean` (`QuarticVertexOrder := Fin S.card ≃ ↥S`,
     `OrderedQuarticWickData`, the fixed-`S` reindexing equivalence
     `quarticWickDiagramEquivOrderedData`), `Fermionic/Diagrammatics/WickDiagram/Amplitude.lean`
     (`quarticWickDiagramAmplitude`, a genuine sum over vertex orders, not an average).
   - PR 6, `Fermionic/Diagrammatics/DysonDiagramExpansion.lean` — **complete, no `sorry`**:
     ...
     `Fermionic/Diagrammatics/QuarticLocalLeg.lean`'s per-leg operator/energy-shift/CAR semantics), plus the
     vertex-order-sum reindexing (`sum_quarticWickDiagram_eq_sum_orderedData`).    - **PR 7 complete through component decomposition and scalar-prefactor factorization:**
