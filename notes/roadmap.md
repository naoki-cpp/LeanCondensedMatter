...
| Second quantization, fermionic primary line (Fock space, CAR, Hamiltonians, Dyson expansion) | D | `stated` (phases 1-7 done; phase 9 in progress — the general finite-temperature n-point Bloch–de Dominicis theorem is proved (`Common/BlochDeDominicis/Induction.lean`), with its combinatorial recursion extracted as a reusable abstract theorem (`Combinatorics/PerfectPairing/FirstPairRecursion.lean`); the genuine interaction-picture Dyson series and diagram connectedness (steps 5-7) are not yet started) | [notes/roadmaps/second-quantization.md](roadmaps/second-quantization.md) |
