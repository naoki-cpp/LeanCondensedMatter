...
theorem reciprocalPairing_isSymm :
  ...
  constructor
  intro x y
  rw [reciprocalPairing_apply, reciprocalPairing_apply, real_inner_comm]
...
`reciprocalPairing`. -/
noncomputable def reciprocalBasis {ι : Type*} [Finite ι] [DecidableEq ι]
    (b : Module.Basis ι ℝ V) : Module.Basis ι ℝ V :=
  (reciprocalPairing (V := V)).dualBasis
...
theorem reciprocalLattice_span_of_basis {ι : Type*} [Finite ι] [DecidableEq ι]
...
/-- The reciprocal of a lattice spanned by a finite real basis is discrete. -/
noncomputable instance instDiscreteTopologyReciprocalLatticeSpan {ι : Type*} [Finite ι]
    (b : Module.Basis ι ℝ V) :
    DiscreteTopology (reciprocalLattice (Submodule.span ℤ (Set.range b))) := by
  classical
  rw [reciprocalLattice_span_of_basis b]
  infer_instance

/-- The reciprocal of a lattice spanned by a finite real basis is again a full `ℤ`-lattice. -/
noncomputable instance instIsZLatticeReciprocalLatticeSpan {ι : Type*} [Finite ι]
    (b : Module.Basis ι ℝ V) :
    IsZLattice ℝ (reciprocalLattice (Submodule.span ℤ (Set.range b))) where
  span_top := by
    classical
    rw [reciprocalLattice_span_of_basis b]
    exact ZSpan.span_top (reciprocalBasis b)

/-- Taking the physical reciprocal lattice twice recovers a lattice generated by a finite real
basis. The same normalized `2π` pairing is used in both reciprocal operations. -/
@[simp]
theorem reciprocalLattice_reciprocalLattice_span_of_basis {ι : Type*} [Finite ι]
    (b : Module.Basis ι ℝ V) :
