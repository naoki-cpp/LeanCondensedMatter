...
This module owns the velocity-based specialization of the generalized transport machinery.
Abstract quantum theory supplies Heisenberg evolution and transport factorization, but it does not
select a position observable or a velocity operator.  Once a concrete one-particle model supplies a
velocity `v` and a localization differential of the form

```text
δₕ(M f) = 1/2 {N (d f), v},
```

a one-body quantity `m` commuting with the localized one-form operators has conventional current

```text
jᵐ = 1/2 {v, m}.
```

Thus the velocity-dependent notion is kept in the first-quantized layer rather than the abstract
`QuantumTheory` layer.
-/
...
/-- Flux functional generated by a concrete one-particle velocity and localization of a one-form-like
test. -/
