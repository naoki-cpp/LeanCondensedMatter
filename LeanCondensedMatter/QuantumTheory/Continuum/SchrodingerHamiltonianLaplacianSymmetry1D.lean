import LeanCondensedMatter.QuantumTheory.Continuum.SchrodingerHamiltonianSymmetry1D
...
private theorem ae_eq_mul_of_l2ToTemperedDistribution_eq_smulLeft
    (u v : ContinuumL2Wavefunction1D) (g : ℝ → ℂ) (hg : Continuous g)
  ...
  have hgvLoc : LocallyIntegrable (fun x => g x * v x) (volume : Measure ℝ) := by
    ...
    exact (hvLoc.locallyIntegrableOn Set.univ).continuousOn_mul hg.continuousOn isLocallyClosed_univ
  ...
  have htestC_smooth : ContDiff ℝ ∞ testC := by
    dsimp [testC]
    fun_prop
  let testS : SchwartzMap ℝ ℂ := htestC_support.toSchwartzMap htestC_smooth
  calc
    ...
    _ = ∫ x, test x • (g x * v x) := by
      simp [l2ToTemperedDistribution1D, testS, testC, mul_assoc, mul_comm, mul_left_comm]
...
theorem fourier_continuumH2Laplacian1D_ae
  ...
  apply ae_eq_mul_of_l2ToTemperedDistribution_eq_smulLeft
    ...
    laplacianFourierSymbol1D continuous_laplacianFourierSymbol1D
  rw [← MeasureTheory.Lp.fourier_toTemperedDistribution_eq,
    ← MeasureTheory.Lp.fourier_toTemperedDistribution_eq]
  rw [fourier_l2ToTemperedDistribution1D_continuumH2Laplacian1D]
...
theorem continuumH2Laplacian1D_symmetric
  ...
  rw [← MeasureTheory.Lp.inner_fourier_eq, ← MeasureTheory.Lp.inner_fourier_eq]
  rw [MeasureTheory.L2.inner_def, MeasureTheory.L2.inner_def]
  apply integral_congr_ae
  filter_upwards [fourier_continuumH2Laplacian1D_ae ψ,
    fourier_continuumH2Laplacian1D_ae φ] with x hψ hφ
  rw [hψ, hφ]
  simp [laplacianFourierSymbol1D, RCLike.inner_apply, mul_assoc, mul_comm]
