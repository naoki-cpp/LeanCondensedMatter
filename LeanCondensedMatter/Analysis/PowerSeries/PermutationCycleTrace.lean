...
This file is the finite bridge from the fixed-label single-cycle combinatorics to the matrix-trace
coefficients used later in W3 of the exchange/cumulant route.

The combinatorial labels are `Fin m`, while `x : Fin m → ι` assigns physical matrix indices to
those labels. Repetitions are deliberately allowed. This distinction is what makes the later
...
universal exchange factor. Cyclic enumeration and the trace identity are proved on top of this
boundary rather than pushed back into the pure combinatorics backend.
-/
