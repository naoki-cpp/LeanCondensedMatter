...
`DependsOnlyOnDifferential d Φ` records the intrinsic statement that `Φ` depends only on `d f`.
`IsDifferentialCurrent d Φ J` is stronger: it states that `J` is a chosen linear current extension
with `Φ(f) = J(d f)`. A strictly stronger
...
transport. This is representation-independent: it does not choose a current extension
`J : OneForm → Obs`. -/
