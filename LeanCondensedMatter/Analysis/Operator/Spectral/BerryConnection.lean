...
private noncomputable def nondegenerateDerivativeCoefficient [DecidableEq ι]
    (eigenbasis : OrthonormalBasis ι ℂ H) (energy : ι → ℝ)
    (hamiltonianDerivative : κ → H →L[ℂ] H) (μ : κ) (m n : ι) : ℂ :=
  if m = n then 0
  else
    inner ℂ (eigenbasis m) (hamiltonianDerivative μ (eigenbasis n)) /
      (((energy n - energy m : ℝ) : ℂ))

private noncomputable def nondegenerateEigenvectorDerivative [DecidableEq ι]
    (eigenbasis : OrthonormalBasis ι ℂ H) (energy : ι → ℝ)
    (hamiltonianDerivative : κ → H →L[ℂ] H) (μ : κ) (n : ι) : H :=
  ∑ m : ι,
    nondegenerateDerivativeCoefficient eigenbasis energy hamiltonianDerivative μ m n •
      eigenbasis m

private theorem repr_nondegenerateEigenvectorDerivative [DecidableEq ι]
    (eigenbasis : OrthonormalBasis ι ℂ H) (energy : ι → ℝ)
    (hamiltonianDerivative : κ → H →L[ℂ] H) (μ : κ) (n k : ι) :
    eigenbasis.repr
        (nondegenerateEigenvectorDerivative eigenbasis energy hamiltonianDerivative μ n) k =
      nondegenerateDerivativeCoefficient eigenbasis energy hamiltonianDerivative μ k n := by
  rw [eigenbasis.repr_apply_apply]
  unfold nondegenerateEigenvectorDerivative
  rw [inner_sum, Finset.sum_eq_single k]
  · simp
  · intro m _ hmk
    rw [inner_smul_right, eigenbasis.inner_eq_zero (Ne.symm hmk)]
    simp
  · simp

/-- Construct pointwise differentiated eigenbasis data from a nondegenerate orthonormal
eigenbasis and self-adjoint Hamiltonian derivatives.

The eigenvector derivative uses the canonical pointwise gauge with vanishing band-diagonal
component and the Born--Fock coefficient in every off-diagonal component. This is a local
pointwise construction only; it does not choose a smooth gauge over parameter space. -/
noncomputable def ofNondegenerate [DecidableEq ι]
  ...
  let eigenvectorDerivative : κ → ι → H :=
    fun μ n =>
      nondegenerateEigenvectorDerivative eigenbasis energy hamiltonianDerivative μ n
  let energyDerivative : κ → ι → ℝ :=
    fun μ n =>
      ContinuousLinearMap.diagonalExpectationValue
        (hamiltonianDerivative μ) (hamiltonianDerivative_selfAdjoint μ) (eigenbasis n)
  have hhamiltonianCoordinate (k : ι) (x : H) :
      eigenbasis.repr (hamiltonian x) k =
        ((energy k : ℝ) : ℂ) * eigenbasis.repr x k := by
    calc
      eigenbasis.repr (hamiltonian x) k =
          inner ℂ (eigenbasis k) (hamiltonian x) :=
        eigenbasis.repr_apply_apply (hamiltonian x) k
      _ = inner ℂ (hamiltonian (eigenbasis k)) x := by
        symm
        exact hamiltonian_selfAdjoint.isSymmetric.apply_clm _ _
      _ = inner ℂ (((energy k : ℝ) : ℂ) • eigenbasis k) x := by
        rw [hamiltonian_eigenvector]
      _ = ((energy k : ℝ) : ℂ) * inner ℂ (eigenbasis k) x := by
        rw [inner_smul_left]
        simp
      _ = ((energy k : ℝ) : ℂ) * eigenbasis.repr x k := by
        rw [eigenbasis.repr_apply_apply]
  have henergyDerivative (μ : κ) (n : ι) :
      (((energyDerivative μ n : ℝ) : ℂ)) =
        inner ℂ (eigenbasis n) (hamiltonianDerivative μ (eigenbasis n)) := by
    simp [energyDerivative, ContinuousLinearMap.coe_diagonalExpectationValue_right]
  have hdifferentiatedEigenpair (μ : κ) (n : ι) :
      hamiltonianDerivative μ (eigenbasis n) +
          hamiltonian (eigenvectorDerivative μ n) =
        (((energyDerivative μ n : ℝ) : ℂ)) • eigenbasis n +
          ((energy n : ℝ) : ℂ) • eigenvectorDerivative μ n := by
    apply eigenbasis.repr.injective
    ext k
    by_cases hkn : k = n
    · subst k
      simp [eigenvectorDerivative, repr_nondegenerateEigenvectorDerivative,
        nondegenerateDerivativeCoefficient, hhamiltonianCoordinate,
        henergyDerivative]
    · have hgap :
          (((energy n - energy k : ℝ) : ℂ)) ≠ 0 := by
        exact_mod_cast sub_ne_zero.mpr (hnondegenerate k n hkn).symm
      simp only [map_add, map_smul, PiLp.add_apply, PiLp.smul_apply]
      rw [eigenbasis.repr_apply_apply, hhamiltonianCoordinate,
        repr_nondegenerateEigenvectorDerivative,
        repr_nondegenerateEigenvectorDerivative]
      rw [eigenbasis.repr_apply_apply, eigenbasis.inner_eq_zero hkn]
      simp only [smul_eq_mul, mul_zero, zero_add]
      simp [nondegenerateDerivativeCoefficient, hkn, hgap]
      field_simp [hgap]
      push_cast
      ring
  have hdifferentiatedOrthonormality (μ : κ) (m n : ι) :
      inner ℂ (eigenvectorDerivative μ m) (eigenbasis n) +
          inner ℂ (eigenbasis m) (eigenvectorDerivative μ n) = 0 := by
    by_cases hmn : m = n
    · subst n
      simp [eigenvectorDerivative, repr_nondegenerateEigenvectorDerivative,
        nondegenerateDerivativeCoefficient, inner_conj_symm]
    · have hgap :
          (((energy n - energy m : ℝ) : ℂ)) ≠ 0 := by
        exact_mod_cast sub_ne_zero.mpr (hnondegenerate m n hmn).symm
      have hgap' :
          (((energy m - energy n : ℝ) : ℂ)) ≠ 0 := by
      ...
      have hstar :
        ...
        calc
          ...
          _ = inner ℂ (eigenbasis m) (hamiltonianDerivative μ (eigenbasis n)) :=
            hamiltonianDerivative_selfAdjoint μ |>.isSymmetric.apply_clm _ _
      rw [inner_conj_symm]
      simp only [eigenvectorDerivative, eigenbasis.repr_apply_apply.symm]
      rw [repr_nondegenerateEigenvectorDerivative,
        repr_nondegenerateEigenvectorDerivative]
      simp [nondegenerateDerivativeCoefficient, hmn, Ne.symm hmn, map_div, hstar]
  ...
  exact
    { hamiltonian := hamiltonian
      ...
      differentiatedEigenpair := hdifferentiatedEigenpair
      differentiatedOrthonormality := hdifferentiatedOrthonormality }
