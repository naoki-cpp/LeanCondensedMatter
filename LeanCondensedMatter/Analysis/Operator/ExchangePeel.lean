...
/-!
# Finite scalar-exchange peel in a complex algebra
...
noncomputable def operatorPeelSum {Label A : Type*} [Semiring A] [Algebra ℂ A]
    (operator : Label → A) (exchangeCoeff : Label → Label → ℂ)
    (ζ : ℂ) (C : Label) : List Label → A
  | [] => 0
  | D :: t =>
      exchangeCoeff C D • (t.map operator).prod +
        ζ • (operator D * operatorPeelSum operator exchangeCoeff ζ C t)

@[simp]
theorem operatorPeelSum_nil {Label A : Type*} [Semiring A] [Algebra ℂ A]
    (operator : Label → A) (exchangeCoeff : Label → Label → ℂ)
    (ζ : ℂ) (C : Label) :
...
ordered product. -/
theorem operator_mul_prod_eq_operatorPeelSum
  ...
  | cons D t ih =>
      simp only [List.map_cons, List.prod_cons, operatorPeelSum, List.length_cons]
