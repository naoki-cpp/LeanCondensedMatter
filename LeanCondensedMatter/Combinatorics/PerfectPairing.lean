...
pairing as a permutation listing its pairs, scalar pairing evaluation, pairing-induced vertex
graphs, relabeling, crossing statistics, and the first-pair erase/insert recursion used by the
Bloch--de Dominicis development. The four-position enumeration is
