...
`Pairing.eraseZeroOrderIso` itself, supplying `k := (pairing.partner 0).pred hj0` (`pairing.partner
0` is always nonzero, `Ne.symm (pairing.partner_ne 0)`) at the one remaining call site the earlier
file's docstring deferred. This is what finally lets `Pairing.eraseZeroPair`'s reindexing line up
term-by-term against `EraseIdxOfFn.lean`'s single-`succAbove` description of list erasure.
-/
