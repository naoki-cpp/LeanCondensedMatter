...
/-- Partitions of `s` with `k + 1` blocks are equivalent to a choice of the block containing
`a` together with a `k`-block partition of its complement. -/
def partsCardSuccEquiv
