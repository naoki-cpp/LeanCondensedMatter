...
theorem freeGibbsExpectation_infiniteDysonCoeff_one
    ...
    freeGibbsExpectation ε β
        (Common.infiniteDysonCoeff (freeEigenvalue ε) V 1 t) =
...
theorem freeGibbsExpectation_infiniteDysonCoeff_one_eq_intervalIntegral
    (ε : Mode → ℝ) (β : ℝ)
    (V : FockSpace Mode →ₗ[ℂ] FockSpace Mode) (t : ℝ) :
    freeGibbsExpectation ε β
        (Common.infiniteDysonCoeff (freeEigenvalue ε) V 1 t) =
      -∫ σ in (0 : ℝ)..t,
        freeGibbsExpectation ε β
          ((interactionPicture ε V σ).comp
            (Common.infiniteDysonCoeff (freeEigenvalue ε) V 0 σ)) := by
  rw [freeGibbsExpectation_infiniteDysonCoeff_one]
  simp_rw [Common.infiniteDysonCoeff_zero, LinearMap.comp_id,
