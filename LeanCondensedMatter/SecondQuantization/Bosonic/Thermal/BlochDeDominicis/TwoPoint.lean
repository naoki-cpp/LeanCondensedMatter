...
theorem tsumTrace_imaginaryTimeEvolveFree_comp_annihilate_comp_create
  ...
  have hcomm : Common.zetaCommutator (1 : ℂ) (annihilate i) (create j) =
      (if i = j then (1 : ℂ) else 0) •
        (LinearMap.id : FockSpace Mode →ₗ[ℂ] FockSpace Mode) := by
    by_cases hij : i = j
    · subst j
      simpa [Common.zetaCommutator, comm] using comm_annihilate_create i i
    · simpa [Common.zetaCommutator, comm, hij] using comm_annihilate_create i j
