...
/-- The vacuum-normalized source moments as a normalized generating functional. -/
noncomputable def vacuumNormalizedFunctional (Z : SourceGeneratingFunctional Source) :
    GeneratingFunctional Source (PowerSeries ℂ) where
  moment := Z.vacuumNormalized
