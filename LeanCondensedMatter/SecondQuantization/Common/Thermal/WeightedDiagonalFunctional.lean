...
/-- **The normalized weighted diagonal coordinate functional**, `Tr_w(A) / weightSum(w)`, bundled
as a complex-linear functional of `A`. No positivity or physical-state interpretation is implied for
...
noncomputable def normalizedWeightedDiagonal (w : Config → ℂ) :
  ...
  map_add' A B := by rw [weightedTrace_add, add_div]
  map_smul' c A := by
