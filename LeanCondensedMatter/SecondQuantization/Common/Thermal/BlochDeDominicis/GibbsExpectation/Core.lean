...
The foundational trace-level identities in `BlochDeDominicis/Unnormalized/TwoPoint.lean`,
`Unnormalized/FourPointReduction.lean`, and `Unnormalized/PeelFirstTrace.lean` are stated in terms of
the *un-normalized* `traceFock`/`tsumTrace` of `e^{-βH₀}X`, deliberately left un-divided so callers
choose whether/how to divide. The genuine physical statement of the theorem — e.g. the 4-point
expansion `⟨C₁C₂C₃C₄⟩_β = ⟨C₁C₂⟩_β⟨C₃C₄⟩_β + ζ⟨C₁C₃⟩_β⟨C₂C₄⟩_β + ⟨C₁C₄⟩_β⟨C₂C₃⟩_β` (for
`ζ = ±1`, the project's physics reference notes, `quantum-statistical-mechanics.tex`) — is a
product of *normalized* 2-point *numbers*, not un-normalized traces. This file introduces that
normalized functional and its basic algebraic properties; the sibling files in this directory —
...
`BlochDeDominicis/Unnormalized/TwoPoint.lean`'s own `tsum` theorem already requires), not a `tsum`
