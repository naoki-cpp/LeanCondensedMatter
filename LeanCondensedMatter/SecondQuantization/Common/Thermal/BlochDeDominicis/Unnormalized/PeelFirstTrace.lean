...
Both the canonical `traceFock` form (`traceFock_diagonalEvolution_comp_peel`) and a `tsum`,
...
only the `tsum` form applies there. The rotated tail's diagonal-series summability isn't a separate
hypothesis — it follows from the double-series hypothesis alone via
`summable_matrixCoeff_diag_comp_of_summable_uncurry`, the same way `TwoPoint.lean`'s `tsum` theorem
derives it.
...
theorem traceFock_diagonalEvolution_comp_peel
...
/-- **The `tsum` trace-level peel-first identity**: the coordinate-summability analogue of
