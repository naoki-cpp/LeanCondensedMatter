...
`GibbsExpectation.lean`'s `gibbsExpectation_four_point` (`n = 2`) to arbitrary `n`. Proved by strong
induction on `n`, following the physics reference notes' own proof strategy: peel the first
operator off the front (`gibbsExpectation_peel_indexed`), apply the inductive hypothesis to each
remaining `2(n-1)`-operator product, and reassemble via `Pairing.equivSigma`
(`Pairing.sum_eq_sum_sum_insertFirstPair`), matching signs via `Pairing.weight_eraseZeroPair` and
the pairs decomposition via `Pairing.prod_pairs_eq_firstPair_mul`.
-/
