...
noncomputable def TwoPointDiagram.interactionVertexComponentEquiv
    ...
    ↥S ≃ Σ B : d.componentPartition.parts,
      ↥(TwoPointDiagram.interactionSector (B : Finset (TwoPointVertex S))) :=
  d.componentPartition.equivSigmaSubfinsets S
    (fun v => (Sum.inr v : TwoPointVertex S))
    (fun _ => Finset.mem_univ _)
    (fun B => TwoPointDiagram.interactionSector (B : Finset (TwoPointVertex S)))
    (fun B => TwoPointDiagram.interactionSector_subset (B : Finset (TwoPointVertex S)))
    (fun B v => TwoPointDiagram.mem_interactionSector_subtype
...
theorem TwoPointDiagram.interactionSector_component_unique
    ...
    (hvB : (v : Fin N) ∈ TwoPointDiagram.interactionSector
      (B : Finset (TwoPointVertex S)))
    (hvC : (v : Fin N) ∈ TwoPointDiagram.interactionSector
