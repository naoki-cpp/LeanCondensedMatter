...
private theorem reachable_ofSlotSplitVacuum_of_reachable {x y : ↥(S \ T)}
...
private theorem exists_eq_slotSplitVacuumVertex_of_adj
...
theorem interactionSector_componentBlock_slotSplitVacuumVertex (v : ↥(S \ T)) :
  ...
  constructor
  · intro hx
    obtain ⟨hxS, hxBlock⟩ := (TwoPointDiagram.mem_interactionSector _ x).1 hx
  ...
  · intro hx
    ...
    apply (TwoPointDiagram.mem_interactionSector _ x).2
