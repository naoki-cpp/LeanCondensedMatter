...
private theorem TwoPointDiagram.externalSlotLegSplitting_external_externalPart
  ...
  convert slotLegSplitting_external
    (interactionSector_subset (d.vertexGraph.componentBlock (Sum.inl 0))) e using 1
...
private theorem TwoPointDiagram.externalSlotLegSplitting_interaction_externalPart
  ...
  convert slotLegSplitting_left_interaction
    (interactionSector_subset (d.vertexGraph.componentBlock (Sum.inl 0))) v l using 1
  · simp only [TwoPointDiagram.externalInteractionPart]
    apply Fin.ext
    rfl
...
private theorem TwoPointDiagram.twoPointLegEquiv_externalPieceLegEquiv_symm
  ...
  | inr p =>
      ...
      apply Prod.ext
      · exact Subtype.ext
          (standardSlotEquivOfCardEq_symm_coe d.externalInteractionPart rfl v)
      · rfl
