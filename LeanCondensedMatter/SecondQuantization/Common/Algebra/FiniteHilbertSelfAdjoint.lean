...
private theorem finiteHilbertOperatorMatrix_apply
  ...
  change
    ((EuclideanSpace.basisFun Config ℂ).repr
      (finiteHilbertOperatorAlgEquiv A (finiteHilbertBasisState n))).ofLp m =
        matrixCoeff A m n
...
theorem star_finiteHilbertOperator_eq_iff_matrixCoeff
    ...
    star (finiteHilbertOperatorAlgEquiv A) = finiteHilbertOperatorAlgEquiv B ↔
  ...
  · intro h m n
    have hlin :
        LinearMap.adjoint (finiteHilbertOperatorAlgEquiv A).toLinearMap =
          (finiteHilbertOperatorAlgEquiv B).toLinearMap := by
  ...
  · intro h
    have hmat :
        LinearMap.toMatrix b b
            (LinearMap.adjoint (finiteHilbertOperatorAlgEquiv A).toLinearMap) =
          LinearMap.toMatrix b b (finiteHilbertOperatorAlgEquiv B).toLinearMap := by
      ext m n
...
private theorem finiteHilbertOperator_isSelfAdjoint_iff_matrix_isHermitian
    (A : AlgebraicFock Config →ₗ[ℂ] AlgebraicFock Config) :
    IsSelfAdjoint (finiteHilbertOperatorAlgEquiv A) ↔
