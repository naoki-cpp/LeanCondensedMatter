...
This module owns the analytic perturbation data and the identification of derivatives of the local
normalized log partition function with coefficients of the formal Dyson logarithm. Concrete
...
omit [LinearOrder Mode] in
private theorem iteratedDeriv_analyticNormalizedLogPartitionFunction_eq_factorial_mul_coeff
    (ε : Mode → ℝ) {β : ℝ} (hβ : 0 ≤ β)
    (V : OccupationFock Mode →ₗ[ℂ] OccupationFock Mode) (n : ℕ) :
