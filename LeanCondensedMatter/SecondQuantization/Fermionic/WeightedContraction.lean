...
in the finite-mode, finite-temperature fermionic Bloch–de Dominicis theorem. The lemma holds for *any*
...
occupation-number-diagonal special case (`weightedTrace`/`normalizedWeightedDiagonal`'s `Σₙ w(n)⟨n|A|n⟩`
...
be nonzero (`WeightedFreeTwoPointFunction.lean`'s `weightedFreeTwoPointFunction`) — that claim is scoped to this
...
weighted trace — hence the normalized weighted diagonal functional — vanish termwise for any occupation-number-
...
theorem normalizedWeightedDiagonal_annihilate_comp_annihilate (w : FermionOccupation Mode → ℂ)
  ...
  rw [normalizedWeightedDiagonal, weightedTrace_annihilate_comp_annihilate, zero_div]

theorem normalizedWeightedDiagonal_create_comp_create (w : FermionOccupation Mode → ℂ) (i j : Mode) :
    normalizedWeightedDiagonal w ((create i).comp (create j)) = 0 := by
  rw [normalizedWeightedDiagonal, weightedTrace_create_comp_create, zero_div]
...
functional of two annihilation operators vanishes, for any occupation-number-diagonal weight `w`. Combines
`imaginaryTimeEvolve_annihilate` (each evolved annihilation operator is a scalar multiple of the
un-evolved one) with `normalizedWeightedDiagonal_annihilate_comp_annihilate` on both time-ordering
branches. -/
