...
Instantiates `Common.CarriesParticleNumberCharge` (`Common/ParticleNumberSelectionRule.lean`) for
...
`Common.CarriesParticleNumberCharge.comp` and
`Common.diagonalCoeff_eq_zero_of_carriesParticleNumberCharge`, this reduces the same-type
contraction vanishing proved by hand in `ThermalContraction.lean` to the general particle-number
selection rule.
-/
...
theorem carriesParticleNumberCharge_annihilate (i : Mode) :
    Common.CarriesParticleNumberCharge
...
a basis state `n` with one fewer particle, `fermionParticleNumber m = fermionParticleNumber n +
1`. -/
