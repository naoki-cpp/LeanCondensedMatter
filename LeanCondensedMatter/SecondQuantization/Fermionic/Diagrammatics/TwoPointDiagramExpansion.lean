...
import LeanCondensedMatter.SecondQuantization.Fermionic.Diagrammatics.TwoPointDiagramExpansion.MixedComponentLocalTimeTransport
...
comparison of component positions and pairs across interaction-time assignments, endpoint agreement
of those transports up to normalized-pair orientation, factorization of mixed-time contraction
products, decomposition of mixed crossing counts, reduction of their parity to complete component-
position inversions and concrete vacuum four-leg event blocks, unconditional factorization of mixed
pairing weights, pairing values, coupling products, fixed-time values, and component-local Dyson
signs, together with the bridge from pair transport to local interaction-time dependence, the
factorized ordered-simplex integrand, and the perturbative coefficient for the imaginary-time two-
point function with quartic interaction vertices.
-/
