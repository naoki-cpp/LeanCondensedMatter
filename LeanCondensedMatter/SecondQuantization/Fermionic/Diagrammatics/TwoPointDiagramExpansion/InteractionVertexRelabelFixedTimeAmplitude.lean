...
import LeanCondensedMatter.SecondQuantization.Fermionic.Diagrammatics.TwoPointDiagramExpansion.InteractionVertexRelabelPairing
...
/-!
# Fixed-time amplitude covariance under interaction-vertex relabeling

For an injective interaction-time assignment, the mixed atomic positions are unchanged by relabeling
and the mixed-order pairing is therefore identical after inverse-precomposing the old time assignment.
This file adds the remaining field-data covariance: the time-labelled field carried by each mixed
position is also identical.  Consequently every density-state pair contraction, the complete pairing
value, and finally the fixed-time Wick-diagram amplitude are invariant under the corresponding
...
/-- At injective interaction times, every mixed atomic position carries exactly the same
 time-labelled field before and after interaction-slot relabeling. -/
...
/-- Each density-state contraction is invariant under the corresponding interaction-slot relabeling
at injective interaction times. -/
...
/-- The fixed-time amplitude of a fixed-external Wick diagram is exactly covariant under
interaction-slot relabeling away from interaction-time diagonals. -/
