...
contraction-integrand factorization required by milestone M1 of the fermionic linked-cluster theorem.
-/
...
integrands of its connected components, evaluated on the corresponding restricted time assignments. -/
theorem QuarticWickDiagram.contractionIntegrand_assembleVertexOrder_eq_prod_components
    ...
    d.contractionIntegrand ε β (d.assembleVertexOrder orders shuffle) τ =
      ∏ B : d.componentPartition.parts,
        (d.restrictComponentConnected B.2).1.contractionIntegrand ε β (orders B)
          (d.componentTimeAssignment shuffle τ B) := by
  classical
  simp only [QuarticWickDiagram.contractionIntegrand,
    QuarticWickDiagram.restrictComponentConnected]
