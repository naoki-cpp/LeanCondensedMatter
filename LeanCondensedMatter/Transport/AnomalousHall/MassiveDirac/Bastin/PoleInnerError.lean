...
The inner-window Lorentzian error estimate previously implemented here is now a private detail of
the generic `Transport.Analysis.LorentzianPole` theorem.  This file is retained temporarily so
historical module paths continue to resolve.
