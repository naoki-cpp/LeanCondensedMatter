...
/-!
# Infinite-cutoff and transverse Born retarded-advanced current rung

This Phase 5 consumer keeps the finite-cutoff longitudinal current-rung API from
`BornCurrentVertexWeakDisorder.lean` canonical and adds only the information not already owned by
that path.  The common real radial denominator integral is removed by the convergent
`pMax → +∞` limit from `BornCurrentVertexIntegral.lean`, and the same shared kernel is used for the
orientation-sensitive `σᵧ` channel.

The continuum disorder is then parameterized by `W(γ) = 4 γ ℏ² v²`.  In the metallic regime
`m² < ε²`, the infinite-cutoff longitudinal coefficient tends to the existing canonical weak-
disorder rung coefficient, while in repository orientation `Gᴿ σₓ Gᴬ`

```text
Y₁ / γ → 2 ε m / (ε² + m²).
```

This module does not replace the fixed-cutoff weak-disorder/transport-lifetime bridge, solve a new
ladder equation, insert the result into Kubo–Středa, claim Ward consistency, or include crossed
diagrams.
-/
