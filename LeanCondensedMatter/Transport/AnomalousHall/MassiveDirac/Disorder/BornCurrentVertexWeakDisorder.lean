...
private theorem hasDerivAt_continuumBornRADenominatorCenter
  ...
  convert
    (hasDerivAt_const p
      ...
      (((hasDerivAt_id p).pow 2).const_mul (v ^ 2)) using 1 <;> ring
...
theorem one_sub_continuumBornRetardedAdvancedPauliXWeakDisorderCurrentRungCoefficient
  ...
  by_cases hsum : probeEnergy ^ 2 + m ^ 2 = 0
  · have hE : probeEnergy = 0 := by nlinarith [sq_nonneg probeEnergy, sq_nonneg m]
    have hm : m = 0 := by nlinarith [sq_nonneg probeEnergy, sq_nonneg m]
    simp [hE, hm]
  · field_simp [hsum]
    ring
