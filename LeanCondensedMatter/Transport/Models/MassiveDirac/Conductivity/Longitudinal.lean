...
conductivity bridge, and its fixed-disorder zero-broadening boundary.
...
zero-broadening limit are owned upstream by `MassiveDirac.Streda`, while generic retarded-advanced
trace identities remain owned by `Transport.Streda`. The conductivity layer starts only where the
common static Bastin/Středa conductivity prefactor and physical continuum momentum normalization are
attached, or where an upstream lifetime is connected to an actual conductivity benchmark.
-/
