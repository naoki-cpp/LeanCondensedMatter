...
self-energy and its polar provenance, `Born.Denominator` owns exact denominator evaluation and UV
behavior, `Born.ZeroBroadening` owns the fixed-cutoff complex boundary, and `Born.Damping` owns the
upper-band damping projection. The finite-`η` Born-Dyson layer propagates that boundary separately,
retaining the finite real self-energy contribution before any weak-disorder limit is taken.

The finite-external-broadening Born-Dyson rung reduces through the shared polar Pauli algebra to an
in-plane coefficient pair `(X,Y)` with repository orientation `[[X,-Y],[Y,X]]`. Its pointwise angular
coefficients expose the corresponding fixed-cutoff zero-broadening boundary before any radial limit
exchange is attempted. Radial integration attaches the scalar-disorder line and physical momentum
measure exactly once before the pair is consumed by the canonical in-plane ladder solution. For the
zero-external-broadening Born route, `BornCurrentVertexRung` owns the exact normalized finite-cutoff
longitudinal rung, while the fixed-cutoff weak-disorder and infinite-cutoff limits remain separate
downstream routes.
