...
private theorem diracEigenvalue_sq
  ...
  have heig :
      hamiltonianOperator v m px py (b i) =
        (((lam : ℝ) : ℂ)) • b i := by
    change
      hamiltonianOperator v m px py (diracEigenbasisFin v m px py i) =
  ...
  have hreal : lam * lam = energySq v m px py := by
...
private theorem hamiltonianOperator_pointwiseEigenbasis
  ...
  · have heig :=
    ...
    simpa [pointwiseEigenbasis, OrthonormalBasis.reindex_apply, bandEnergy] using heig
  · have heig :=
      (hamiltonianOperator_isSelfAdjoint v m px py).isSymmetric.apply_eigenvectorBasis
...
/-- Generic pointwise Berry data for the nondegenerate massive-Dirac Hamiltonian.

The two derivative directions are the physical momentum directions. The construction is local at
the chosen momentum and makes no global smooth-gauge, Berry-phase, Chern-number, or topology claim. -/
