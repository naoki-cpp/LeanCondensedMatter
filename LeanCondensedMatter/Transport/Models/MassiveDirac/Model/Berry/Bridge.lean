...
Their antisymmetrization in an ordered pair of in-plane directions is the natural oriented
Berry-curvature component. The familiar two-dimensional scalar Berry curvature is the `(x,y)`
...
/-- Oriented two-band Berry-curvature component obtained from the antisymmetric force-matrix
numerator for an ordered in-plane direction pair. -/
def forceMatrixBerryCurvatureComponent
    (μ ν : Direction2) (band : Band) (v m px py : ℝ) : ℝ :=
  (forceMatrixAntisymmetricNumerator μ ν band v m px py).im /
...
theorem forceMatrixBerryCurvature_eq_berryCurvature (band : Band) (v m px py : ℝ)
  ...
  rw [forceMatrixBerryCurvature, forceMatrixBerryCurvatureComponent,
    forceMatrixAntisymmetricNumerator_xy_im band v m px py hE,
