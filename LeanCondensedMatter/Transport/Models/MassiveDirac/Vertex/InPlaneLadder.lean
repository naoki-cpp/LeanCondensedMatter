...
`Gᴿ Γ Gᴬ`, the rung is one direction-indexed coefficient vector `(X,Y)` acting through the matrix
...
the rung, fixed-point, and uniqueness statements are vector equations. This file does not define
the Born-Dyson momentum integrals that supply the rung, take any broadening/disorder limit, identify
a transport lifetime, or insert the result into conductivity.
-/
...
/-- Repository-oriented isotropic in-plane matrix derived from rung vector `(X,Y)`. -/
...
/-- Convergence of the rung vector propagates to the shifted-ladder determinant. -/
theorem tendsto_inPlaneLadderDeterminant
  ...
  have hcomponents := hrung
  rw [tendsto_pi_nhds] at hcomponents
  have hx := hcomponents .x
  have hy := hcomponents .y
...
/-- Convergence of the rung vector propagates to the solved ladder vector whenever the limiting
...
theorem tendsto_inPlaneLadderSolvedVector
    ...
    Tendsto (fun a => inPlaneLadderSolvedVector (rung a)) l
      (nhds (inPlaneLadderSolvedVector rung₀)) := by
...
theorem inPlaneLadder_fixedPoint_unique
  ...
  have hxFixed :
      coefficients .x = 1 + (rung .x * coefficients .x - rung .y * coefficients .y) := by
