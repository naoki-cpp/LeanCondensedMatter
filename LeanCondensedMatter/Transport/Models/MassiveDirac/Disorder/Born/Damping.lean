...
This module consumes the fixed-cutoff complex Born zero-broadening boundary, extracts the scalar and
`σ_z` channel imaginary limits, and propagates them through the physical continuum prefactor and the
upper-band Fermi-surface projection. The resulting positive damping energy is
...
theorem tendsto_finiteCutoffContinuumBornZIntegral_im_broadening_zero
  ...
  have hchannel := tendsto_finiteCutoffContinuumBornZIntegral_broadening_zero
    side v m probeEnergy pMax hvelocity hmetal hcutoff
  have him := (Complex.continuous_im.tendsto
    ((m : ℂ) * finiteCutoffContinuumBornDenominatorIntegralZeroBroadeningBoundary
      side v m probeEnergy pMax)).comp hchannel
  simpa [finiteCutoffContinuumBornDenominatorIntegralZeroBroadeningBoundary,
    Complex.mul_im] using him
...
theorem tendsto_finiteCutoffContinuumBornScalarIntegral_im_broadening_zero
  ...
  have hchannel := tendsto_finiteCutoffContinuumBornScalarIntegral_broadening_zero
    side v m probeEnergy pMax hvelocity hmetal hcutoff
  have him := (Complex.continuous_im.tendsto
    ((probeEnergy : ℂ) * finiteCutoffContinuumBornDenominatorIntegralZeroBroadeningBoundary
      side v m probeEnergy pMax)).comp hchannel
  simpa [finiteCutoffContinuumBornDenominatorIntegralZeroBroadeningBoundary,
    Complex.mul_im] using him
...
theorem tendsto_finiteCutoffContinuumBornScalarSelfEnergyCoefficient_im_broadening_zero
  ...
  have hchannel := tendsto_finiteCutoffContinuumBornScalarIntegral_broadening_zero
    side v m probeEnergy pMax hvelocity hmetal hcutoff
  have hscaled := (tendsto_const_nhds.mul hchannel : Tendsto
    (fun broadening : ℝ =>
      ((disorderStrength * continuumBornAngularMeasurePrefactor hbar : ℝ) : ℂ) *
        finiteCutoffContinuumBornScalarIntegral
  ...
  have him := (Complex.continuous_im.tendsto
    (((disorderStrength * continuumBornAngularMeasurePrefactor hbar : ℝ) : ℂ) *
      ((probeEnergy : ℂ) *
        finiteCutoffContinuumBornDenominatorIntegralZeroBroadeningBoundary
          side v m probeEnergy pMax))).comp hscaled
  simpa [finiteCutoffContinuumBornDenominatorIntegralZeroBroadeningBoundary,
    Complex.mul_im] using him
