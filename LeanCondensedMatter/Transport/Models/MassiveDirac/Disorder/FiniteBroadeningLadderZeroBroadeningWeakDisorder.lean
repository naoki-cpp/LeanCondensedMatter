...
The solved vector exposes the weak-disorder dressed-current factor before downstream conductivity
classification, while the ladder action supplies the correction relative to the bare source.
...
converges to the longitudinal dressed-current factor
`2 (ε² + m²) / (ε² + 3 m²)` with vanishing raw transverse component. This is the repository-oriented
weak-disorder realization of the diagonal factor `F` in Ado et al., EPL 111, 37004 (2015), Eq. (11).
-/
theorem tendsto_finiteCutoffContinuumBornDysonLadderSolvedVectorZeroBroadeningBoundary_disorder_zero
  ...
  have hden : probeEnergy ^ 2 + 3 * m ^ 2 ≠ 0 := by
    nlinarith [sq_pos_of_ne_zero (ne_of_gt hprobe), sq_nonneg m]
  have hκ : 1 - κ ≠ 0 := by
    simpa [κ] using weakDisorderTargetOneMinusRung_ne_zero m probeEnergy hmetal
  have hfactorReal :=
  ...
  have hfactor :
    ...
    have hcast := congrArg Complex.ofReal hfactorReal
    push_cast at hcast
    simpa [κ] using hcast
  have htarget :
      inPlaneLadderSolvedVector targetRung =
        inPlaneCoefficientVector
          (((2 * (probeEnergy ^ 2 + m ^ 2) /
            (probeEnergy ^ 2 + 3 * m ^ 2) : ℝ) : ℂ)) 0 := by
    rw [show targetRung = inPlaneCoefficientVector κ 0 by rfl]
    rw [inPlaneLadderSolvedVector_zero_transverse κ hκ, hfactor]
  rw [htarget] at hsolved
  exact hsolved
