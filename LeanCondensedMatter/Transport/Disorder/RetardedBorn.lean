...
Green operator is not identified with the second-order Born Green truncation by definition: equality
requires the explicit retarded specialization of `BornClosureHypothesis`.

No advanced Born closure, self-consistent Born approximation, Dyson-resummed Born Green operator,
dressed propagator inside the self-energy, vertex correction, Ward identity, trace-per-volume
...
/-- Conventional retarded name for the canonical side-indexed second-order Born Green truncation.
It is deliberately not a Dyson-resummed Green operator and does not require centering. -/
noncomputable def secondOrderBornRetardedGreen
    (energy broadening : ℝ) : H →L[ℂ] H :=
  ensemble.secondOrderBornGreen .retarded energy broadening

@[simp]
theorem secondOrderBornGreen_retarded
    (energy broadening : ℝ) :
    ensemble.secondOrderBornGreen .retarded energy broadening =
      ensemble.secondOrderBornRetardedGreen energy broadening :=
  rfl
...
/-- Exact decomposition of the averaged retarded Green operator into the named second-order Born
Green truncation plus its closure error. Centering is used only through the exact averaged-Dyson
reduction. -/
theorem averagedRetardedGreen_eq_secondOrderBornGreen_add_error
    (hcentered : ensemble.IsCentered)
    (energy broadening : ℝ) (hbroadening : 0 < broadening) :
    ensemble.averagedRetardedGreen energy broadening =
      secondOrderBornRetardedGreen ensemble energy broadening +
  ...
  unfold secondOrderBornRetardedGreen bornRetardedClosureError bornClosureError secondOrderBornGreen
...
/-- Equality with the second-order Born Green truncation follows only after supplying both centered
disorder and the explicit closure hypothesis. -/
theorem averagedRetardedGreen_eq_secondOrderBornGreen
    (hcentered : ensemble.IsCentered)
    (energy broadening : ℝ) (hbroadening : 0 < broadening)
    (closure : RetardedBornClosureHypothesis ensemble energy broadening) :
    ensemble.averagedRetardedGreen energy broadening =
      secondOrderBornRetardedGreen ensemble energy broadening := by
  rw [averagedRetardedGreen_eq_secondOrderBornGreen_add_error
    ensemble hcentered energy broadening hbroadening]
