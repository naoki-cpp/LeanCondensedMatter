...
/-!
# Zero-temperature Lorentzian Fermi-edge analysis

This module owns the interaction between the strict zero-temperature occupation and the scalar
Lorentzian approximate identity. It records local occupied/unoccupied windows, exact finite-window
truncation at the Fermi level, arctangent formulas, broadening limits, and the isolated-pole weights
`π`, `0`, and `π / 2`.

The scalar Fermi step itself remains in `Transport.Analysis.ZeroTemperatureOccupation`.
-/
