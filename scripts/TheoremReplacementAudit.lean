...
private def markdown (entries : Array AuditEntry) : String := Id.run do
  ...
  text := text ++
    "Candidates are advisory and proof-guided: a source theorem must already occur in the target " ++
    "proof term. Lean then replays the source with `apply`; every remaining goal must close from an " ++
    "existing target hypothesis or typeclass synthesis. This intentionally favors precision and " ++
    "repository refactoring value over global theorem-search recall.\n\n"
  for entry in entries do
    if !entry.definitionallyEquivalentTo.isEmpty || !entry.replacementCandidates.isEmpty then
      ...
      if !entry.definitionallyEquivalentTo.isEmpty then
        text := text ++ "Definitionally equivalent proof dependencies:\n\n"
