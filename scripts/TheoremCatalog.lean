...
private def markdown (entries : Array CatalogEntry) (chains : Array (Array String)) : String := Id.run do
  ...
  output := output ++ "Each row is one theorem with all relevant audit attributes shown together; no separate candidate-set intersection is required.\n\n"
  for entry in directWrappers do
    let target := entry.directWrapperOf.getD "<unknown>"
    let targetModule := entry.directWrapperTargetModule.getD "<unknown>"
    output := output ++ s!"- `{entry.name}` → `{target}` — module `{entry.moduleName}`; target module: `{targetModule}`; compiled consumers: {entry.compiledConsumerCount}; single consumer: {entry.singleCompiledConsumer}; terminal: {entry.terminal}; cross-module: {entry.crossModuleDirectWrapper}; retained single-consumer: {entry.singleConsumerRetainedMention}\n"
...
run_cmd do
  ...
  logInfo m!"Generated theorem catalog with {catalog.size} declarations, {dependencyEdgeCount catalog} dependency edges, and {terminals.size} terminal theorems ({completedTerminals.size} mentioned in completed.md; {terminalsWithCompiledConsumers.size} with compiled project consumers; {priorityReviewQueue.size} priority review candidates; {retainedSingleConsumers.size} retained single-consumer theorems; {singleConsumerReviewQueue.size} single-consumer review candidates; {directWrappers.size} direct-wrapper candidates; {crossModuleDirectWrappers.size} cross-module direct-wrapper candidates; {chains.size} multi-step single-consumer chains)"
